class FlatfileImporter::Importer
  ...
  def self.import_excel(organisation, file)
    pi = ProductImporter.new(file)
    pi.import_to(organisation)
  end
  ...
  def process_line(line)
    ...
    unless primary_key.blank?
      ...
      if !primary_record
        ...
        primary_record = primary_record_endpoint.where(primary_key_attribute => primary_key).first
        if primary_record
